...
6. The `mesosphereReactClient` passes this token down to your mesosphere backend to
   validate
7. Your mesosphere backend retrieves the public key from {props.provider} to check
   that the token's signature is valid.
8. The `mesosphereReactClient` is notified of successful authentication, and
   {props.integrationProvider} now knows that the user is authenticated with
   mesosphere. `usemesosphereAuth` returns `isAuthenticated: true` and the
