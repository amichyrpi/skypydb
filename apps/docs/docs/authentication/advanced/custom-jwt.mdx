...
sidebar_position: 4
description:
  "Configure mesosphere to work with custom JWT providers that don't implement full
...
is signed (the mesosphere deployment can tell the information is really from the
...
[OIDC Provider](/authentication/advanced/custom-auth) entry in `mesosphere/auth.config.ts`. If
...
to configure a Custom JWT provider in the `mesosphere/auth.config.ts` file.
...
mesosphere clients to implement token refreshing.
...
- `applicationID`: mesosphere will verify that JWTs have this value in the `aud`
...
[Custom OIDC Providers](/authentication/advanced/custom-auth#client-side-integration) for
how to do this.
