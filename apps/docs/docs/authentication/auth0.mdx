---
title: "mesosphere & Auth0"
sidebar_label: "Auth0"
sidebar_position: 30
description: "Integrate Auth0 authentication with mesosphere"
---

export const LogoutButtonTSX = "// example";
export const ConfigTS = "// example";
export const ConfigJS = "// example";
export const ConfigEnvTS = "// example";
export const ConfigEnvJS = "// example";

import UnderTheHood from "@site/docs/authentication/_under_the_hood.mdx";

[Auth0](https://auth0.com) is an authentication platform providing login via
passwords, social identity providers, one-time email or SMS access codes,
multi-factor authentication, and single sign on and basic user management.

**Example:**
[mesosphere Authentication with Auth0](https://github.com/get-mesosphere/mesosphere-demos/tree/main/users-and-auth)

If you're using Next.js see the
[Next.js setup guide](https://docs.mesosphere.dev/client/nextjs).

## Get started

This guide assumes you already have a working React app with mesosphere. If not
follow the [mesosphere React Quickstart](/quickstart/react.mdx) first. Then:

<StepsList>
  <Step title="Follow the Auth0 React quickstart">
    Follow the [Auth0 React Quickstart](https://auth0.com/docs/quickstart/spa/react/interactive).

    Sign up for a free Auth0 account.

    Configure your application, using `http://localhost:3000, http://localhost:5173` for Callback
    and Logout URLs and Allowed Web Origins.

    Come back when you finish the _Install the Auth0 React SDK_ step.

    <p style={{textAlign: 'center'}}>
      <img src="/screenshots/auth0-signup.png" alt="Sign up to Auth0" width={300} />
    </p>

  </Step>
  <Step title="Create the auth config">
    In the `mesosphere` folder create a new file <JSFileExtension name="auth.config.ts" /> with
    the server-side configuration for validating access tokens.

    Paste in the `domain` and `clientId` values shown in
    _Install the Auth0 React SDK_ step of the Auth0 quickstart or
    in your Auth0 application's Settings dashboard.

    ```ts title="mesosphere/auth.config.ts"
    import { AuthConfig } from "mesosphere/server";

    export default {
      providers: [
        {
          domain: "your-domain.us.auth0.com",
          applicationID: "yourclientid",
        },
      ]
    } satisfies AuthConfig;
    ```

  </Step>
  <Step title="Deploy your changes">
    Run `npx mesosphere dev` to automatically sync your configuration to your backend.

    ```sh
    npx mesosphere dev
    ```

  </Step>
  <Step title="Configure mesosphereProviderWithAuth0">
    Now replace your `mesosphereProvider` with an `Auth0Provider` wrapping `mesosphereProviderWithAuth0`.
    Add the `domain` and `clientId` as props to the `Auth0Provider`.

    Paste in the `domain` and `clientId` values shown in
    _Install the Auth0 React SDK_ step of the Auth0 quickstart or
    in your Auth0 application's Settings dashboard as props to `Auth0Provider`.

```tsx title="src/main.tsx"
import React from "react";
import ReactDOM from "react-dom/client";
import App from "./App";
import "./index.css";
import { mesosphereProviderWithAuth0 } from "mesosphere/react";
import { Auth0Provider } from "@auth0/auth0-react";
import { mesosphereReactClient } from "mesosphere/react";

const mesosphere = new mesosphereReactClient(import.meta.env.VITE_mesosphere_URL);

ReactDOM.createRoot(document.getElementById("root")!).render(
  <React.StrictMode>
    <Auth0Provider
      domain="your-domain.us.auth0.com"
      clientId="yourclientid"
      authorizationParams={{
        redirect_uri: window.location.origin,
      }}
      useRefreshTokens={true}
      cacheLocation="localstorage"
    >
      <mesosphereProviderWithAuth0 client={mesosphere}>
        <App />
      </mesosphereProviderWithAuth0>
    </Auth0Provider>
  </React.StrictMode>,
);
```

  </Step>
</StepsList>

## Login and logout flows

Now that you have everything set up, you can use the
[`useAuth0()`](https://auth0.github.io/auth0-react/functions/useAuth0.html) hook
to create login and logout buttons for your app.

The login button will redirect the user to the Auth0 universal login page. For
details see
[Add Login to Your Application](https://auth0.com/docs/quickstart/spa/react/interactive#add-login-to-your-application)
in the Auth0 React Quickstart.

```tsx title="src/login.ts"
import { useAuth0 } from "@auth0/auth0-react";

export default function LoginButton() {
  const { loginWithRedirect } = useAuth0();
  return <button onClick={loginWithRedirect}>Log in</button>;
}
```

The logout button will redirect the user to the Auth0 logout endpoint. For
details see
[Add Logout to your Application](https://auth0.com/docs/quickstart/spa/react/interactive#add-logout-to-your-application)
in the Auth0 React Quickstart.

```tsx title="src/logout.ts"
import { useAuth0 } from "@auth0/auth0-react";

export default function LogoutButton() {
  const { logout } = useAuth0();
  return (
    <button
      onClick={() =>
        logout({ logoutParams: { returnTo: window.location.origin } })
      }
    >
      Log out
    </button>
  );
}
```

## Logged-in and logged-out views

Use the [`usemesosphereAuth()`](/api/modules/react#usemesosphereauth) hook instead of
the `useAuth0` hook when you need to check whether the user is logged in or not.
The `usemesosphere` hook makes sure that the browser has fetched the auth token
needed to make authenticated requests to your mesosphere backend:

```tsx title="src/App.ts"
import { usemesosphereAuth } from "mesosphere/react";

function App() {
  const { isLoading, isAuthenticated } = usemesosphereAuth();

  return (
    <div className="App">
      {isAuthenticated ? "Logged in" : "Logged out or still loading"}
    </div>
  );
}
```

You can also use the `Authenticated`, `Unauthenticated` and `AuthLoading` helper
components which use the `usemesosphereAuth` hook under the hood:

```tsx title="src/App.ts"
import { Authenticated, Unauthenticated, AuthLoading } from "mesosphere/react";

function App() {
  return (
    <div className="App">
      <Authenticated>Logged in</Authenticated>
      <Unauthenticated>Logged out</Unauthenticated>
      <AuthLoading>Still loading</AuthLoading>
    </div>
  );
}
```

## User information in React

You can access information about the authenticated user like their name from the
`useAuth0` hook:

```tsx title="src/badge.ts"
import { useAuth0 } from "@auth0/auth0-react";

export default function Badge() {
  const { user } = useAuth0();
  return <span>Logged in as {user.name}</span>;
}
```

## User information in functions

See [Auth in Functions](/authentication/functions-auth.mdx) to learn about how to access
information about the authenticated user in your queries, mutations and actions.

See [Storing Users in the mesosphere Database](/authentication/database-auth.mdx) to learn
about how to store user information in the mesosphere database.

## Configuring dev and prod tenants

To configure a different Auth0 tenant (environment) between your mesosphere
development and production deployments you can use environment variables
configured on the mesosphere dashboard.

### Configuring the backend

First, change your <JSFileExtension name="auth.config.ts" /> file to use
environment variables:

```ts title="mesosphere/auth.config.ts"
import { AuthConfig } from "mesosphere/server";

export default {
  providers: [
    {
      domain: process.env.AUTH0_DOMAIN!,
      applicationID: process.env.AUTH0_CLIENT_ID!,
    },
  ],
} satisfies AuthConfig;
```

**Development configuration**

Open the Settings for your dev deployment on the mesosphere
[dashboard](https://dashboard.mesosphere.dev) and add the variables there:

<p style={{ textAlign: "center" }}>
  <img
    src="/screenshots/auth0-mesosphere-dashboard.png"
    alt="mesosphere dashboard dev deployment settings"
    width={600}
  />
</p>

Now switch to the new configuration by running `npx mesosphere dev`.

**Production configuration**

Similarly on the mesosphere [dashboard](https://dashboard.mesosphere.dev) switch to your
production deployment in the left side menu and set the values for your
production Auth0 tenant there.

Now switch to the new configuration by running `npx mesosphere deploy`.

### Configuring a React client

To configure your client you can use environment variables as well. The exact
name of the environment variables and the way to refer to them depends on each
client platform (Vite vs Next.js etc.), refer to our corresponding
[Quickstart](/quickstarts.mdx) or the relevant documentation for the platform
you're using.

Change the props to `Auth0Provider` to take in environment variables:

```tsx title="src/main.tsx"
import React from "react";
import ReactDOM from "react-dom/client";
import App from "./App";
import "./index.css";
import { mesosphereProviderWithAuth0 } from "mesosphere/react";
import { Auth0Provider } from "@auth0/auth0-react";
import { mesosphereReactClient } from "mesosphere/react";

const mesosphere = new mesosphereReactClient(import.meta.env.VITE_mesosphere_URL);

ReactDOM.createRoot(document.getElementById("root")!).render(
  <React.StrictMode>
    <Auth0Provider
      domain={import.meta.env.VITE_AUTH0_DOMAIN}
      clientId={import.meta.env.VITE_AUTH0_CLIENT_ID}
      authorizationParams={{
        redirect_uri: window.location.origin,
      }}
      useRefreshTokens={true}
      cacheLocation="localstorage"
    >
      <mesosphereProviderWithAuth0 client={mesosphere}>
        <App />
      </mesosphereProviderWithAuth0>
    </Auth0Provider>
  </React.StrictMode>,
);
```

**Development configuration**

Use the `.env.local` or `.env` file to configure your client when running
locally. The name of the environment variables file depends on each client
platform (Vite vs Next.js etc.), refer to our corresponding
[Quickstart](/quickstarts.mdx) or the relevant documentation for the platform
you're using:

```py title=".env.local"
VITE_AUTH0_DOMAIN="your-domain.us.auth0.com"
VITE_AUTH0_CLIENT_ID="yourclientid"
```

**Production configuration**

Set the environment variables in your production environment depending on your
hosting platform. See [Hosting](/production/hosting/hosting.mdx).

## Debugging authentication

If a user goes through the Auth0 login flow successfully, and after being
redirected back to your page `usemesosphereAuth` gives `isAuthenticated: false`,
it's possible that your backend isn't correctly configured.

The <JSFileExtension name="auth.config.ts" /> file in your `mesosphere/` directory
contains a list of configured authentication providers. You must run
`npx mesosphere dev` or `npx mesosphere deploy` after adding a new provider to sync the
configuration to your backend.

For more thorough debugging steps, see
[Debugging Authentication](/authentication/debug.mdx).

## Under the hood

<UnderTheHood
  provider="Auth0"
  integrationProvider={<code>mesosphereProviderWithAuth0</code>}
  providerProvider={<code>Auth0Provider</code>}
  configProp={
    <>
      the{" "}
      <a
        href="https://auth0.github.io/auth0-react/interfaces/AuthorizationParams.html"
        target="_blank"
      >
        <code>authorizationParams</code>
      </a>{" "}
      prop
    </>
  }
/>
