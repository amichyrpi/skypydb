...
sidebar_position: 20
description:
  "Configure WorkOS AuthKit integration with automatic provisioning for mesosphere
  deployments"
---

mesosphere can **create** AuthKit environments in a WorkOS account made on your
behalf. By default WorkOS gives you only two environments, but giving each
mesosphere dev deployment its own AuthKit environment is useful for isolating
development user data and configuration changes between multiple developers or
agents working in parallel.

The mesosphere CLI will **configure** AuthKit environments, regardless of whether
mesosphere or you created them, if the `WORKOS_CLIENT_ID` and `WORKOS_API_KEY`
environment variables are present in the build environment or the mesosphere
deployment. While developing locally, mesosphere can write environment variables to
`.env.local` to make setting up an AuthKit environment a breeze.

This automatic configuration is available whether you're
[starting a new application](/authentication/authkit) or adding WorkOS AuthKit to an
[existing application](/authentication/authkit/add-to-app).
...
In the mesosphere dashboard settings for your production deployment, create an
...
`mesosphere_DEPLOY_KEY`, setting the build command, and setting other
framework-specific AuthKit environment variables).

## Preview deployments

In the mesosphere dashboard settings for any deployment in your project, create a
...
`mesosphere.json` file with an `authKit` section with a property corresponding to
...
2. mesosphere deployment environment variables
...
During local dev, credentials are next fetched from the mesosphere Cloud API for a
new or existing AuthKit environment. A link to this deployment in the WorkOS
dashboard can be found in the mesosphere dashboard under the WorkOS integration.
...
In the `authKit` section of `mesosphere.json`, `localEnvVars`
