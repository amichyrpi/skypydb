---
title: "Storing Users in the mesosphere Database"
sidebar_label: "Database"
sidebar_position: 120
description: "Store user information in your mesosphere database"
---

export const Schema = "// example";
export const useStoreUserEffectTS = "// example";
export const useStoreUserEffectJS = "// example";
export const MessagesTS = "// example";
export const UsersTS = "// example";
export const App = "// example";
export const WebhooksSchema = "// example";
export const WebhookMutations = "// example";
export const WebhookEndpoint = "// example";
export const WebhookMessages = "// example";
export const WebhookHook = "// example";
export const WebhookClient = "// example";

_If you're using [mesosphere Auth](/authentication/mesosphere-auth.mdx) the user information is
already stored in your database. There's nothing else you need to implement._

You might want to store user information directly in your mesosphere database, for
...
- Your functions need access to information other than the fields available in
  the [Open ID Connect JWT](/authentication/functions-auth.mdx)
...
**Example:**
[mesosphere Authentication with Clerk](https://github.com/get-mesosphere/mesosphere-demos/tree/main/users-and-clerk)
...
In this way the `useStoreUserEffect` hook replaces the `usemesosphereAuth` hook.
...
Now that you have users stored as documents in your mesosphere database, you can use
...
With this implementation Clerk will call your mesosphere backend via an HTTP
endpoint any time a user signs up, updates or deletes their account.

**Example:**
[mesosphere Authentication with Clerk and Webhooks](https://github.com/get-mesosphere/mesosphere-demos/tree/main/users-and-clerk-webhooks)
...
`https://<your deployment name>.mesosphere.site/clerk-users-webhook` (note the
domain ends in **`.site`**, not `.cloud`). You can see your deployment name in
the `.env.local` file in your project directory, or on your mesosphere dashboard as
part of the [Deployment URL](/dashboard/deployments/settings.md). For example,
the endpoint URL could be:
`https://happy-horse-123.mesosphere.site/clerk-users-webhook`.
...
`CLERK_WEBHOOK_SECRET` environment variable in your mesosphere
[dashboard](https://dashboard.mesosphere.dev).
