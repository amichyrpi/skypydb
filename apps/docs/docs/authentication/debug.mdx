...
description: "Troubleshoot authentication issues in mesosphere"
---
...
previously, it takes some time for the client to authenticate with the mesosphere
backend. Therefore on page load, `ctx.auth.getUserIdentity()` called within a
query returns `null`.

To handle this, you can either:

1. Use the `Authenticated` component from `mesosphere/react` to wrap the component
   that includes the `useQuery` call (see the last two steps in the
   [Clerk guide](/authentication/clerk.mdx#get-started))
...
2. Then call this function from whichever client you're using to talk to mesosphere.

3. Open the
   [logs page on your dashboard](https://dashboard.mesosphere.dev/deployment/logs).
...
4. What do you see on the logs page?
   ...
   **Answer: I don't see anything**:
   ...
   - Potential cause: Your client is not connected to mesosphere. Check your client
   ...
   - Potential cause: The code has not been pushed. For dev deployments make
     sure you have `npx mesosphere dev` running. For prod deployments make sure you
     successfully pushed via `npx mesosphere deploy`. Go to the _Functions_ page on
...
2. Check the token
   - For Websocket-based clients (`mesosphereReactClient` and `mesosphereClient`),
   ...
   - For HTTP based clients (`mesosphereHTTPClient` and the
     HTTP API), select `Fetch/XHR` as the type of traffic.
...
3. Do you see the authentication token in the traffic?
   ...
   **Answer: No**:
   - Potential cause: The mesosphere client is not configured to get/fetch a JWT
     token. You're not using
     `mesosphereProviderWithClerk`/`mesosphereProviderWithAuth0`/`mesosphereProviderWithAuth`
     with the `mesosphereReactClient` or you forgot to call `setAuth` on
     `mesosphereHTTPClient` or `mesosphereClient`.
   - Potential cause: You are not signed in, so the token is `null` or
     `undefined` and the `mesosphereReactClient` skipped authentication altogether.
...
1. Open the _Settings_ > _Authentication_ on your dashboard. What do you see?
   ...
   **Answer: I see
   ...
   - Cause: You do not have an `auth.config.ts` (or `auth.config.js`) file in
     your `mesosphere` directory, or you haven't pushed your code. Follow the
     authentication guide to create a valid auth config file. For dev
     deployments make sure you have `npx mesosphere dev` running. For prod
     deployments make sure you successfully pushed via `npx mesosphere deploy`.
...
2. Look at the `iss` field in the JWT token payload at https://jwt.io/. Does it
   ...
   **Answer: No, I don't see the `iss` URL on the mesosphere dashboard**:
   - Potential cause: You copied the wrong value into your
     <JSFileExtension name="auth.config.ts" />
   ...
   - Potential cause: Your client is misconfigured:
     - Clerk: You have the wrong `publishableKey` configured. The key must
       ...
       <JSFileExtension name="auth.config.ts" />. - Also make sure that the JWT
       token in Clerk is called `mesosphere`, as that's the name
       `mesosphereProviderWithClerk` uses to fetch the token!
   ...
   **Answer: No, I don't see the `aud` value in the _Application ID_ field**:
   ...
   - Potential cause: Your client is misconfigured:
     ...
     - Auth0: You have the wrong `clientId` configured. Make sure you're using
       ...
       your <JSFileExtension name="auth.config.ts" />.
     - Custom: Make sure that your client is correctly configured to match your
       <JSFileExtension name="auth.config.ts" />.
