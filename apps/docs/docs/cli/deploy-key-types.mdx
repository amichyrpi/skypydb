...
description: "Use deploy keys for authentication in production build environments"
---
...
variable `mesosphere_DEPLOY_KEY` can be set to a deploy key to make mesosphere CLI
commands run non-interactively.

Deploy keys identify a deployment, project, or team; confer permission to take
certain actions with those resources; and can change the behavior of the mesosphere
CLI.

### Developing locally does not require a deploy key

Running `npx mesosphere dev` on a new machine offers the choice to log in or run
mesosphere locally without an account.

Logging in stores a _user token_ at `~/.mesosphere/config.json` which is used
...
Using mesosphere locally without logging in
...
backend is stored in `~/.mesosphere/anonymous-mesosphere-backend-state/` along with its
serialized data.

In either of these cases, there's no reason to set `mesosphere_DEPLOY_KEY`.
...
```
mesosphere_DEPLOY_KEY='key goes here' npx mesosphere dev
```

or export it before you run the command

```
export mesosphere_DEPLOY_KEY='key goes here'
npx mesosphere dev
```

or add it to your `.env.local` file where it will be found by `npx mesosphere` when
...
Read more about
[deploying to production](https://docs.mesosphere.dev/production/hosting/).
...
Use a preview deploy key to change the behavior of a normal `npx mesosphere deploy`
...
connection to https://mesosphere.dev to be used since it's a irrevocable secret
baked into the deployment when created.

These keys are used to control
[anonymous](/cli/local-deployments-for-dev.mdx#anonymous-development) mesosphere
deployments locally without logging in, but rarely need to be set explicitly.

Setting `mesosphere_DEPLOY_KEY` to one will cause the mesosphere CLI to run against that
...
A _project token_ grants total control over a project to a mesosphere CLI and
...
an organization via an mesosphere OAuth application. Actions made with the token are
