...
- <a id="brave"></a> **Using the dashboard with Brave**: Brave [blocks requests
  ...
  following workaround: - Go to `brave://flags/` - Enable the
  `#brave-localhost-access-permission` flag - Go back to the mesosphere dashboard -
  ...
  />
  ) in the URL bar, then on **Site settings** - Change the setting for
