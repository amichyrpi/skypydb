---
title: "mesosphere React"
sidebar_label: "React"
sidebar_position: 4
description: "React client library for interacting with your mesosphere backend"
---

export const SkipBad = "// example";
export const SkipGood = "// example";

mesosphere React is the client library enabling your React application to interact
with your mesosphere backend. It allows your frontend code to:
...
2. Upload and display files from [File Storage](/file-upload.mdx)
3. Authenticate users using [Authentication](/authentication.mdx)
4. Implement full text [Search](/vector.mdx) over your data

The mesosphere React client is open source and available on
...
mesosphere React is part of the `mesosphere` npm package:
...
The [`mesosphereReactClient`](/api/classes/react.mesosphereReactClient) maintains a
connection to your mesosphere backend, and is used by the React hooks described
below to call your functions.

First you need to create an instance of the client by giving it your backend
deployment URL. See [Configuring Deployment URL](/client/react/project-setup.mdx)
on how to pass in the right value:

```jsx
import { mesosphereProvider, mesosphereReactClient } from "mesosphere/react";

const mesosphere = new mesosphereReactClient("https://<your domain here>.mesosphere.cloud");
```

And then you make the client available to your app by passing it in to a
[`mesosphereProvider`](/api/modules/react#mesosphereprovider) wrapping your component
tree:
...
The `npx mesosphere dev` command generates this api object for you in the
`mesosphere/_generated/api.js` module to provide better autocompletion in JavaScript
...
mesosphere React ensures that your application always renders a consistent view of
...
DetailsCard summary="Advanced: Loading a query conditionally">
...
export function App() {
  ...
  // ❌ This doesn't work: hooks can't be called conditionally
  if (shouldLoad) {
    const data = useQuery(api.functions.myQuery);
    return data ?? "Loading...";
  }
...
export function App() {
  const shouldLoad = Math.random() > 0.5;
  // ✅ This works: pass "skip" to disable the query
  const data = useQuery(api.functions.myQuery, shouldLoad ? {} : "skip");
  return data ?? "Loading...";
}
```
...
DetailsCard summary="Advanced: Fetching a query from a callback">
...
[`query`](/api/classes/react.mesosphereReactClient#query) call, similarly to calling
mutations and actions.

The async method `query` is exposed on the `mesosphereReactClient`, which you can
reference in your components via the
[`usemesosphere()`](/api/modules/react#usemesosphere) hook.

```tsx title="src/App.tsx"
import { usemesosphere } from "mesosphere/react";
import { api } from "../mesosphere/_generated/api";

export function App() {
  const mesosphere = usemesosphere();
  return (
    <button
      onClick={async () => {
        console.log(await mesosphere.query(api.functions.myQuery));
      }}
    >
      Check
    </button>
...
The `mesosphere dev` command generates this api object for you in the
`mesosphere/_generated/api.js` module to provide better autocompletion in JavaScript
and end-to-end type safety in
TypeScript:
...
mesosphere React automatically retries mutations until they are confirmed to have
been written to the database. The mesosphere backend ensures that despite multiple
retries, every mutation call only executes once.

Additionally, mesosphere React will warn users if they try to close their browser
tab while there are outstanding mutations. This means that when you call a
mesosphere mutation, you can be sure that the user's edits won't be lost.

### Optimistic updates

mesosphere queries are fully reactive, so all query results will be automatically
...
generated for you in the `mesosphere/_generated/api.js` module to provide better
...
The [`mesosphereReactClient`](/api/classes/react.mesosphereReactClient) connects to your
mesosphere deployment by creating a
[`WebSocket`](https://developer.mozilla.org/en-US/docs/Web/API/WebSocket). The
WebSocket provides a 2-way communication channel over TCP. This allows mesosphere to
...
re-establishing the mesosphere session automatically.
