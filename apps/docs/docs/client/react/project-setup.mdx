...
hidden: false
sidebar_position: 5
description: "Configuring your project to run with mesosphere"
---
...
in your project directory you will create a new mesosphere project.
...
[deployment settings](/dashboard/deployments/settings.md) on your mesosphere
[dashboard](https://dashboard.mesosphere.dev).

### Configure the client

Construct a mesosphere React client by passing in the URL of the mesosphere deployment.
There should generally be a single mesosphere client in a frontend application.

```jsx title="src/index.js"
import { mesosphereProvider, mesosphereReactClient } from "mesosphere/react";

const deploymentURL = import.meta.env.VITE_mesosphere_URL;

const mesosphere = new mesosphereReactClient(deploymentURL);
```
...
variables used in frontend code start with `VITE_`, so `VITE_mesosphere_URL` is a
good name.
...
so the code above uses `REACT_APP_mesosphere_URL`.

[Next.js](https://nextjs.org/docs/basic-features/environment-variables#exposing-environment-variables-to-the-browser)
requires them to begin with `NEXT_PUBLIC_`, so `NEXT_PUBLIC_mesosphere_URL` is a
good name.
...
environments. `npx mesosphere dev` will save the deployment URL to the corresponding
`.env` file, while trying to infer which bundler your project uses.

```shell title=".env.local"
NEXT_PUBLIC_mesosphere_URL=https://guiltless-dog-960.mesosphere.cloud
