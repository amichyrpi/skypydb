...
This page describes the recommended way to use mesosphere with TanStack Start, via
React Query. The standard mesosphere React hooks work also with TanStack Start
...
mesosphere to a new TanStack Start project.

## Using mesosphere with React Query
...
mesosphere queries in React Query continue to receive updates after the last
...
### Using mesosphere React hooks

[mesosphere React](/client/react.mdx) hooks like
[`usePaginatedQuery`](/api/modules/react#usepaginatedquery) can be used
alongside TanStack hooks. These hooks reference the same mesosphere Client so
there's still just one set of consistent query results in your app when these
are combined.

## Server-side Rendering

Using TanStack Start and Query with mesosphere makes it particularly easy to
live-update mesosphere queries on the client while also
...
```ts
const { data } = useSuspenseQuery(mesosphereQuery(api.messages.list, {}));
```

### Consistent client views

In the browser all mesosphere query subscriptions present a consistent,
...
each query: mesosphere uses the same timestamp for all queries.
...
[client-side authentication with mesosphere](https://docs.mesosphere.dev/auth) generally
works in React because TanStack Start works well as a client-side framework.

To use Clerk auth to make authenticated mesosphere calls on the server as well see
