...
function createAuthorAgent(
  ...
  return new Agent(components.agent, {
    ...
    languageModel: model,
    tools: {
      // See https://docs.mesosphere.dev/agents/tools
...
implementation in
[threads.ts](https://github.com/get-mesosphere/agent/blob/main/example/mesosphere/threads.ts).

See
[chat/basic.ts](https://github.com/get-mesosphere/agent/blob/main/example/mesosphere/chat/basic.ts)
or
[chat/streaming.ts](https://github.com/get-mesosphere/agent/blob/main/example/mesosphere/chat/streaming.ts)
...
import { saveMessage } from "@mesosphere-dev/agent";
...
import { openai } from "@ai-sdk/openai";
import { z } from "zod/v3";
import { Agent, createTool, type Config } from "@mesosphere-dev/agent";
import { components } from "./_generated/api";
...
const sharedDefaults = {
  ...
  // Used for fetching context messages. See https://docs.mesosphere.dev/agents/context
  contextOptions,
  // Used for storing messages. See https://docs.mesosphere.dev/agents/messages
  storageOptions,
  // Used for tracking token usage. See https://docs.mesosphere.dev/agents/usage-tracking
  usageHandler: async (ctx, args) => {
    const { usage, model, provider, agentName, threadId, userId } = args;
    // ... log, save usage to your database, etc.
  },
  // Used for filtering, modifying, or enriching the context messages. See https://docs.mesosphere.dev/agents/context
...
const supportAgent = new Agent(components.agent, {
  ...
  tools: {
    // mesosphere tool. See https://docs.mesosphere.dev/agents/tools
    mymesosphereTool: createTool({
      description: "My mesosphere tool",
      args: z.object({...}),
