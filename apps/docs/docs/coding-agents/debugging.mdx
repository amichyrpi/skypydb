...
logs with
[Log Streaming](https://docs.mesosphere.dev/production/integrations/log-streams/) to
...
[schema](https://github.com/get-mesosphere/agent/blob/main/src/component/schema.ts).
...
`npx mesosphere dev` to generate code for the component. The types expected by the
library are in the npm library, and the types for `components.agent` currently
come from generated code in your project (via `npx mesosphere dev`).

### Circular dependencies

Having the return value of workflows depend on other mesosphere functions can lead
