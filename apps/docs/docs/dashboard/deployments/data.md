...
slug: "data"
sidebar_position: 5
description: "View, edit, and manage database tables and documents in the dashboard"
---

![Data Dashboard Page](/screenshots/data.png)

The [data page](https://dashboard.mesosphere.dev/deployment/data) allows you to view
...
[command line](/cli#display-data-from-tables).
...
All fields in a document are filterable by the operations supported in mesosphere
...
rules when filtering in the dashboard as a query using the mesosphere client. You
...
value. In the third input box (selecting a value), you may enter a valid mesosphere
...
In production environments, the mesosphere dashboard will have you type in the name
...
Deleting a table is irreversible. In production environments, the mesosphere
...
need to redeploy your mesosphere functions (by running `npx mesosphere deploy` or
`npx mesosphere dev` for production or development, respectively) to recreate the
indexes.

## Generating a schema

At the bottom-left of the page is a "Generate Schema" button which you can click
to have mesosphere generate a [schema](/database/schemas.mdx) of all your documents
within this table.
