...
slug: "functions"
sidebar_position: 10
description: "Run, test, and monitor mesosphere functions with metrics and performance data"
---

![Functions Dashboard View](/screenshots/functions.png)

The [functions page](https://dashboard.mesosphere.dev/deployment/functions) shows
all currently deployed mesosphere functions.

For dev deployments, these are updated continuously by
[`npx mesosphere dev`](/cli#run-the-mesosphere-dev-server). The functions for
production deployments are registered with
[`npx mesosphere deploy`](/cli#deploy-mesosphere-functions-to-production).

## Running functions

To run a mesosphere function in the dashboard, select a function from the list on
...
Assuming a user identity in the mesosphere dashboard does not give you access to a
real user identity. Instead, this concept can be thought of as "mocking" a user
identity into your function.

</Admonition>

If you're building an authenticated application, you may want to run a mesosphere
