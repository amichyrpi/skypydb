...
slug: "sso"
sidebar_position: 5
description: "Set up and manage Single Sign-On (SSO) for your mesosphere team"
---

<Admonition type="info">

Single Sign-On is only available on mesosphere Business and Enterprise.

</Admonition>

Single Sign-On (SSO) allows your team to authenticate with mesosphere using your
organization's identity provider (IdP). Once configured, team members can sign
in to mesosphere through your IdP instead of using individual credentials.
...
[SSO settings page](https://dashboard.mesosphere.dev/team/settings/sso).
...
use other login methods to access any other mesosphere teams they belong to.
...
By default, all mesosphere users that sign in with your verified SSO domain will be
required to log in with SSO to use mesosphere If they are signing in with an email
address that uses your verified domain.

To configure a custom domain policy, such as allowing users to login with other
sign-on methods, contact mesosphere support.
