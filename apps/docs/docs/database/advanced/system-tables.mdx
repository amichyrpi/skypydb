...
sidebar_position: 1
description:
  "Access metadata for mesosphere built-in features through system tables including
  scheduled functions and file storage information."
---

System tables enable read-only access to metadata for built-in mesosphere features.
...
- `"_storage"` table contains metadata for
  [stored files](/file-upload/file-metadata.mdx)
