...
sidebar_position: 85
description: "Backup and restore your mesosphere data and files"
---

mesosphere supports Backup & Restore of data via the
[dashboard](https://dashboard.mesosphere.dev/deployment/settings/backups).
...
<ProfessionalFeatureOnly feature="Periodic backups" state="require" />
...
- Use `npx mesosphere dev` to push a known version of good code.
- Use `npx mesosphere env` or the dashboard to restore to a good set of env vars
...
[command line](/cli#export-data-to-a-file):

```sh
npx mesosphere export --path ~/Downloads
```

The backup comes as a generated a ZIP file with all documents in all mesosphere
...
Deployments on mesosphere Professional plan can have many backups with standard
usage based pricing.
...
[usage dashboard](https://dashboard.mesosphere.dev/team/settings/usage). Check the
...
1. Your deployment's code and configuration. mesosphere functions, crons.ts,
