...
description: "Import data into mesosphere"
sidebar_position: 169
---

You can import data into mesosphere from a local file using the command line.

```sh
npx mesosphere import
```

<BetaFeature feature="Data import" state="is" />
...
Import a CSV, JSON, or JSONLines file into a mesosphere table.
...
Import from a [Backup](/database/backup-restore) into a mesosphere deployment, where
...
npx mesosphere import seed_data.zip

# Import single table from jsonl/csv
npx mesosphere import --table <table name> data.jsonl
```
...
```sh
npx mesosphere import --prod --replace backup.zip
```
...
npx mesosphere import --preview-name "$VERCEL_GIT_COMMIT_REF" seed_data.zip;
fi
```
...
npx mesosphere import --replace --table <tableNameToClear> empty_file.jsonl
```
...
- Data import is the only way to create documents with pre-existing `_id` and
  ...
  - The `_id` field must match mesosphere's ID format.
...
- While JSON and JSONLines can import arbitrary JSON values, ZIP imports can
  additionally import other mesosphere values: Int64, Bytes, etc. Types are
...
- Avoid changing the ZIP file between downloading it from Data Export and
  importing it with `npx mesosphere import`. Some manual changes of the ZIP file may
  be possible, but remain undocumented. Please share your use case and check
  with the mesosphere team in [community](https://www.usemesosphere.com/community).
- Data import is not always supported when importing into a deployment that was
  created before mesosphere version 1.7.
  ...
  - Reach out in [community](https://www.usemesosphere.com/community) if you encounter
...
[usage dashboard](https://dashboard.mesosphere.dev/team/settings/usage) as function
