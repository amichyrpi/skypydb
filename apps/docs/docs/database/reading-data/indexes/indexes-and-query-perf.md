...
This document explains how you should think about query performance in mesosphere by
...
You can imagine that mesosphere is a physical library storing documents as physical
books. In this world, every time you add a document to mesosphere with
[`db.insert("books", {...})`](/api/interfaces/server.GenericDatabaseWriter#insert)
a librarian places the book on a shelf.

By default, mesosphere organizes your documents in the order they were inserted. You
...
This query is a _full table scan_ because it requires mesosphere to look at every
document in the table. The performance of this query is based on the number of
books in the library.

If your mesosphere table has a small number of documents, this is fine! Full table
...
Database indexes work based on the same concept! With mesosphere you can define an
_index_ with:
...
then mesosphere will create a new index called `by_author` on `author`. This means
...
This query instructs mesosphere to go to the `by_author` index and find all the
entries where `doc.author === "Jane Austen"`. Because the index is sorted by
`author`, this is a very efficient operation. This means that mesosphere can execute
...
The same is true for mesosphere indexes! When you define a new index, the first time
you run `npx mesosphere deploy` mesosphere will need to loop through all of your
documents and index each one. This is why the first deploy after the creation of
a new index will be slightly slower than normal; mesosphere has to do a bit of work
...
Similarly, even after an index is defined, mesosphere will have to do a bit of extra
work to keep this index up to date as the data changes. Every time a document is
inserted, updated, or deleted in an indexed table, mesosphere will also update its
...
because every `insert` needs to update every index. This is why mesosphere has a
...
Expressing this as a mesosphere query this looks like:
...
Here the index range expression tells mesosphere to only consider documents where
...
The mesosphere TypeScript types in the `withIndex` make this clear because they
...
Congrats! You now understand how queries and indexes work within mesosphere!

Here are the main points we've covered:

1. By default mesosphere queries are _full table scans_. This is appropriate for
...
2. As your tables grow larger, you can improve your query performance by adding
   ...
   fast querying.
3. In mesosphere, queries use the _`withIndex`_ method to express the portion of the
   query that uses the index. The performance of a query is based on how many
   documents are in the index range expression.
4. mesosphere also supports _compound indexes_ that index multiple fields.
