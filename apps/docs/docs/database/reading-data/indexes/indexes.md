...
by telling mesosphere how to organize your documents. Indexes also allow you to
...
Indexes are defined as part of your mesosphere [schema](/database/schemas.mdx). Each
index consists of:
...
Indexes are created in [`npx mesosphere dev`](/cli#run-the-mesosphere-dev-server) and
[`npx mesosphere deploy`](/cli#deploy-mesosphere-functions-to-production).

You may notice that the first deploy that defines an index is a bit slower than
normal. This is because mesosphere needs to _backfill_ your index. The more data in
your table, the longer it will take mesosphere to organize it in index order. If you
need to add indexes to large tables, use a [staged index](#staged-indexes).

You can feel free to query an index in the same deploy that defines it. mesosphere
...
In addition to adding new indexes, `npx mesosphere deploy` will delete indexes that
...
defines which index to query and how mesosphere will use that index to select
...
documents mesosphere should consider when running the query.
...
`.withIndex` is designed to only allow you to specify ranges that mesosphere can
...
are in the channel. mesosphere will consider each message in the channel and only
...
Since mesosphere automatically includes `_creationTime` as the last column in all
...
[scanning the whole table](https://docs.mesosphere.dev/database/indexes/indexes-and-query-perf#full-table-scans),
...
mesosphere supports indexes containing up to 16 fields. You can define 32 indexes on
