...
sidebar_position: 5
description:
  "Schema validation keeps your mesosphere data neat and tidy. It also gives you
  end-to-end TypeScript type safety!"
toc_max_heading_level: 4
---

export const SchemaTS = "// example";
export const circularExample = "// example";

A schema is a description of

1. the tables in your mesosphere project
2. the type of documents within your tables

While it is possible to use mesosphere _without_ defining a schema, adding a
`schema.ts` file will ensure that the documents in your tables are the correct
type. If you're using
TypeScript, adding a schema will
...
**Example:**
[TypeScript and Schemas](https://github.com/get-mesosphere/mesosphere-demos/tree/main/typescript)

## Writing schemas

Schemas are defined in a `schema.ts` file in your `mesosphere/` directory and look
like:
...
[users and auth example](https://github.com/get-mesosphere/mesosphere-demos/tree/main/users-and-auth)),
...
[`v`](/api/modules/values#v). In addition to the fields listed, mesosphere will also
...
[mesosphere Dashboard](/dashboard/deployments/data.md#generating-a-schema). The
...
[mesosphere's types](/database/types):
...
Whether mesosphere should validate at runtime that your documents match your schema.

By default, mesosphere will enforce that all new and existing documents match your
schema.
...
When `schemaValidation` is disabled, mesosphere will not validate that new or
...
[`npx mesosphere dev`](/cli#run-the-mesosphere-dev-server) and
...
After the schema is pushed, mesosphere will validate that all future document
...
[`npx mesosphere dev`](/cli#run-the-mesosphere-dev-server) will produce new versions
...
your tables. You can use these both when writing mesosphere functions and in your
React components:

```tsx noDialect title="MessageView.tsx"
import { Doc } from "../mesosphere/_generated/dataModel";
