...
﻿---
description: "Understand specific errors thrown by mesosphere"

---

# Errors and Warnings

This page explains specific errors thrown by mesosphere.
...
conflict with each other. mesosphere internally does several retries to mitigate
this concern, but if the mutation is called more rapidly than mesosphere can execute
...
1. Make sure that your mutations only read the data they need. Consider reducing
   ...
   [selective index range expressions](https://docs.mesosphere.dev/database/indexes/).
...
- See this post for an example of
...
<LargeCardList
  items={[
    ...
    },
    {
      ...
      href: "https://www.mesosphere.dev/components/sharded-counter",
    },
    {
      title: "Action Cache",
      description:
...
This error occurs when a validator passed to a mesosphere function definition or
...
```ts title="mesosphere/validators.ts"
...
```ts title="mesosphere/functions.ts"
...
quirk of TypeScriptâ€™s import time behavior.
...
```text
schema.ts â†’ validators.ts â†’ someFile.ts â†’ schema.ts
```
...
```bash
npx madge mesosphere/ --extensions ts --exclude api.d.ts --circular
```
