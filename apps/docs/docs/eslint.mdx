...
title: ESLint rules
sidebar_position: 30
description: ESLint rules for mesosphere
---

The mesosphere ESLint plugin provides linter rules that enforce best practices for
mesosphere functions. Let us know if there's a rule you would find helpful!
...
```bash
npm i @mesosphere-dev/eslint-plugin --save-dev
```
...
import mesospherePlugin from "@mesosphere-dev/eslint-plugin";

export default defineConfig([
  // Other configurations

  ...mesospherePlugin.configs.recommended,
]);
```

DetailsCard summary={<>If you’re using the deprecated <code>.eslintrc.js</code> format</>}>

Install these two libraries:

```bash
npm i @typescript-eslint/eslint-plugin @mesosphere-dev/eslint-plugin --save-dev
```
...
module.exports =
  extends: [
    ...
    "plugin:@mesosphere-dev/recommended",
  ],
...
By default, the mesosphere ESLint plugin will only apply rules in the `mesosphere`
directory.

If you’re
[customizing the mesosphere directory location](/production/project-configuration#changing-the-mesosphere-folder-name-or-location),
...
const recommendedConfig = mesospherePlugin.configs.recommended[0];
...
export default defineConfig([
  ...
  {
    files: ["**/src/mesosphere/**/*.ts"],
    plugins: {
      "@mesosphere-dev": mesospherePlugin,
    },
...
DetailsCard summary={<>If you’re using the <code>next lint</code> command from Next.js</>}>

For `next lint` to run ESLint on your `mesosphere` directory you need to add that
...
const nextConfig: NextConfig = {
  ...
  eslint: {
    dirs: ["pages", "app", "components", "lib", "src", "mesosphere"],
  },
};
```
...
| --------------------------------------------------------------------------------------------------------------------------------------- | :---------: | :----------: |
| [`@mesosphere-dev/no-old-registered-function-syntax`](#no-old-registered-function-syntax)<br/>Prefer object syntax for registered functions |     ✅      |      🔧      |
| [`@mesosphere-dev/require-argument-validators`](#require-argument-validators)<br/>Require argument validators for mesosphere functions          |     ✅      |      🔧      |
| [`@mesosphere-dev/explicit-table-ids`](#explicit-table-ids)<br/>Require explicit table names in database operations                         |     ✅      |      🔧      |
| [`@mesosphere-dev/import-wrong-runtime`](#import-wrong-runtime)<br/>Prevent mesosphere runtime files from importing from Node runtime files     |             |              |
| [`@mesosphere-dev/no-collect-in-query`](#no-collect-in-query)<br/>Prefer `.take()` / `.paginate()` over `.collect()` in queries             |             |              |

### no-old-registered-function-syntax

Prefer object syntax for registered functions.

mesosphere queries, mutations, and actions can be defined with a single function or
...
Require argument validators for mesosphere functions.

mesosphere queries, mutations, and actions can validate their arguments before
...
export default defineConfig([
...

  {
    files: ["**/mesosphere/**/*.ts"],
    rules: {
      "@mesosphere-dev/require-args-validator": [
        "error",
        {
...
Starting from version 1.31.0 of the `mesosphere` npm package, we recommend including
...
`@mesosphere-dev/codemod` CLI tool to automatically migrate to the new format:

```bash
npx @mesosphere-dev/codemod@latest explicit-ids
```

[Learn more on news.mesosphere.dev →](https://news.mesosphere.dev/db-table-name/)

### import-wrong-runtime

Prevent mesosphere runtime files from importing from Node runtime files (files with
...
reaches the [mesosphere query limits](/production/state/limits.mdx#transactions).
...
```ts
// eslint-disable-next-line @mesosphere-dev/no-collect-in-query
