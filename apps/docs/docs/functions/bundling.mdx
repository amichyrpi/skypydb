...
title: "Bundling"
sidebar_position: 90
description: "How mesosphere bundles and optimizes your function code"
---

Bundling is the process of gathering, optimizing and transpiling the JS/TS
source code of [functions](/functions.mdx) and their dependencies. During
development and when deploying, the code is transformed to a format that mesosphere
[runtimes](/functions/runtimes.mdx) can directly and efficiently execute.

mesosphere currently bundles all dependencies automatically, but for the Node.js
runtime you can disable bundling certain packages via the
[external packages](#external-packages) config.

## Bundling for mesosphere

When you push code either via `npx mesosphere dev` or `npx mesosphere deploy`, the
mesosphere CLI uses [esbuild](https://esbuild.github.io/) to traverse your `mesosphere/`
...
DetailsCard summary="ESM vs. CJS">
ESM - Is the standard for browser JavaScript - Uses static imports via the `import` and `export` **keywords** (not functions)
at the global scope - Also supports dynamic imports via the asynchronous `import` function
...
The total size of your bundled function code in your `mesosphere/` folder is
...
<StepsList>
  <Step title="Make sure you're using the most recent version of mesosphere">
    ```sh
    npm install mesosphere@latest
    ```
  </Step>
  <Step title="Generate the bundle">
   Note that this will not push code, just generate a bundle for debugging purposes.
    ...
    ```sh
    npx mesosphere dev --once --debug-bundle-path /tmp/myBundle
    ```
...
Code bundled for the mesosphere runtime will be in the `isolate` directory while
...
[default mesosphere runtime](/functions/runtimes.mdx#default-mesosphere-runtime) and
...
imports will always fail in mesosphere.

DetailsCard summary="Examples of libraries with dynamic dependencies">
...
As a workaround for the bundling limitations above, mesosphere provides an escape
hatch: **external packages**. This feature is currently exclusive to mesosphere's
...
your mesosphere modules will rely on the underlying system having that dependency
...
Create a [`mesosphere.json`](/production/project-configuration.mdx#mesospherejson) file
...
The mesosphere CLI searches for external packages within your local `node_modules`
...
If there is a package that you would like working in your mesosphere functions,
