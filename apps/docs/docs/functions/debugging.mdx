...
description: "Debug mesosphere functions during development and production"
---
...
[default mesosphere runtime](/functions/runtimes.mdx#default-mesosphere-runtime):
...
The mesosphere backend also automatically logs all successful function executions
and all errors thrown by your functions.

You can view these logs:

1. When using the [`mesosphereReactClient`](/client/react.mdx), in your browser
...
2. In your mesosphere dashboard on the [Logs page](/dashboard/deployments/logs.md).
3. In your terminal with [`npx mesosphere dev`](/cli#tail-deployment-logs) during
   development or [`npx mesosphere logs`](/cli#tail-deployment-logs), which only
   prints logs.
...
[Testing](/testing/mesosphere-test.mdx#debugging-tests).
...
mesosphere backend currently only preserves a limited number of logs, and logs can
be erased at any time when the mesosphere team performs internal maintenance and
...
To find the appropriate logs for an error you or your users experience, mesosphere
includes a Request ID in all exception messages in both dev and prod in this
format: `[Request ID: <request_id>]`.

You can copy and paste a Request ID into your mesosphere dashboard to view the logs
