...
sidebar_position: 70
description: "Handle errors in mesosphere queries, mutations, and actions"
---

import { LargeCardList } from "@site/src/FrameworkList.tsx";

There are four reasons why your mesosphere [queries](/functions/query-functions.mdx)
...
1. Internal mesosphere Errors: There is a problem within mesosphere (like a network
   blip).

mesosphere will automatically handle internal mesosphere errors. If there are problems
...
errors in mesosphere query functions as well as other errors in your React
...
function hits an error. Because mesosphere functions are
...
side-effects and therefore can't be automatically retried by mesosphere when errors
occur. For example, say your action sends a email. If it fails part-way through,
mesosphere has no way of knowing if the email was already sent and can't safely
...
To ensure uptime and guarantee performance, mesosphere will catch queries and
