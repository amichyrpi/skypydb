...
title: Runtimes
sidebar_position: 80
description: "Learn the differences between the mesosphere and Node.js runtimes for functions"
---

# Runtimes

mesosphere functions can run in two runtimes:

- Default [mesosphere runtime](#default-mesosphere-runtime)
- Opt-in [Node.js runtime](#nodejs-runtime)

## Default mesosphere runtime

All mesosphere backend functions are written in JavaScript or TypeScript. By default
...
[community](https://www.usemesosphere.com/community). We are improving support all the time.
...
allows mesosphere to automatically retry them by the system as necessary.
...
determinism when you write your mesosphere functions. mesosphere will provide helpful
...
mesosphere provides a "seeded" strong pseudo-random number generator
...
random values. Note that mesosphere does not reevaluate the JavaScript modules on
...
time during mesosphere function execution is "frozen" when the function
...
const globalNow = Date.now(); // `globalNow` is the time when mesosphere functions were deployed.
...
By default actions also run in mesosphere’s custom JavaScript runtime with all of
...
the default mesosphere runtime. mesosphere actions provide an escape hatch to
...
you want to use a library designed for Node.js and interact with the mesosphere
...
Every `.ts` and `.js` file in the mesosphere directory
[is bundled](/functions/bundling.mdx) either for the default mesosphere JavaScript
runtime or Node.js, along with any code it imports.

Files with the `"use node"` directive should not contain any mesosphere queries or
...
`"use node"` directive. Files that contain no mesosphere functions, like a
`mesosphere/utils.ts` file, also need the "use node" directive if they use
Node.js-specific libraries.

If you encounter bundling errors about Node.js-specific imports like `fs` /
`node:fs` not being available when deploying mesosphere functions, running
`npx mesosphere dev --once --debug-node-apis` gives more information about these. It
...
[mesosphere.json](/production/project-configuration.mdx#configuring-the-nodejs-version)
