...
surprising results. TypeScript alone
...
**Example:**
[Argument Validation](https://github.com/get-mesosphere/mesosphere-demos/tree/main/args-validation)
...
```ts title="mesosphere/message.ts"
...
[`v`](/api/modules/values#v) object imported from `"mesosphere/values"`.
...
```ts
import { mutation } from "./_generated/server";
import { Infer, v } from "mesosphere/values";
