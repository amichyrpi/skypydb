...
<Admonition type="info">
  mesosphere Platform APIs are in openly available in Beta. Please contact
  platforms@mesosphere.dev if your use case requires additional capabilities.
</Admonition>

This guide is for products that want to orchestrate multiple mesosphere projects in
...
These guides assume a good understanding of mesosphere cloud hierarchy (teams,
...
from your
[Team Settings](https://dashboard.mesosphere.dev/team/settings/access-tokens).

<Admonition type="caution">
  These tokens are owned by the team member that's logged into the mesosphere
...
This means your users authorize your product to manage their own mesosphere team or
projects.
...
call the relevant APIs to manage mesosphere projects and deployments.
...
mesosphere CLI. The mesosphere CLI manages a lot of the heavy lifting: bundling code,
...
Set [`mesosphere_DEPLOY_KEY`](/cli/deploy-key-types.mdx) is the value returned by
the [Create deploy key](/management-api/create-deploy-key) API.

### Pushing code to the dev mesosphere backend

```bash
mesosphere_DEPLOY_KEY="YOUR_DEPLOY_KEY" npx mesosphere dev --once
```

### Pushing code to the prod mesosphere backend

```bash
mesosphere_DEPLOY_KEY="YOUR_DEPLOY_KEY" npx mesosphere deploy
```
