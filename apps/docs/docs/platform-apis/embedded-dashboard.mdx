...
>
  ...
  Application](/platform-apis/oauth-applications) (for mesosphere deployments
  that are in the mesosphere account of the end user), or deploy keys [created
  with the Management API](/management-api/create-deploy-key) (if the mesosphere
  deployment is in a mesosphere account that the end user doesn’t control).
</Admonition>
