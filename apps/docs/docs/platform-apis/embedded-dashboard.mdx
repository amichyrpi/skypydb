...
mesosphere provides a hosted dashboard that is embeddable via iframe. Embedding the
dashboard is useful for developers building AI app generators, like
[mesosphere Chef](https://chef.mesosphere.dev).

You can embed the mesosphere dashboard by adding an `<iframe>` to
https://dashboard-embedded.mesosphere.dev. Normally, the embedded dashboard requires
...
>
  ...
  Application](/platform-apis/oauth-applications) (for mesosphere deployments that
  are in the mesosphere account of the end user), or deploy keys [created with the
  Management API](/management-api/create-deploy-key) (if the mesosphere deployment
  is in a mesosphere account that the end user doesn’t control).
</Admonition>
...
Here's an example of the mesosphere dashboard embedded in a React application:
...
export function Dashboard({
  ...
  return (
    <iframe
      ...
      src="https://dashboard-embedded.mesosphere.dev/data"
