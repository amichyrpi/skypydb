...
mesosphere allows third-party app developers to manage a user's projects on their
...
with mesosphere, which can be done in your
[Team Settings](https://dashboard.mesosphere.dev/team/settings/applications/oauth-apps).
...
application once they have authorized you to access their mesosphere team or
...
To have your application be verified and be accessible for all mesosphere users, it
...
The mesosphere team will respond to your verification request by email if more
information is required.

## Implementing OAuth

mesosphere implements [OAuth 2.0](https://oauth.net/2/) (RFC 6749)'s
[Authorization Code Grant](https://oauth.net/2/grant-types/authorization-code/)
flow. mesosphere also optionally supports the
[PKCE extension](https://oauth.net/2/pkce/) (RFC 7636) to improve security.

- mesosphere provides two _authorization endpoint_ URLs, depending on whether you
  are generating team-scoped or project-scoped tokens:
  - `https://dashboard.mesosphere.dev/oauth/authorize/team`
  - `https://dashboard.mesosphere.dev/oauth/authorize/project`
- mesosphere's **token endpoint** is `https://api.mesosphere.dev/oauth/token`.
...
## Step 1: Redirect the user to mesosphere's authorization endpoint

In your application, navigate the user to:

`https://dashboard.mesosphere.dev/oauth/authorize[TOKEN_SCOPE]?client_id=[CLIENT_ID]&redirect_uri=[REDIRECT_URI]&response_type=code&state=[STATE]`
...
`https://api.mesosphere.dev/oauth/token`
...
- `client_secret` is your app's secret assigned by mesosphere.
...
If all provided values are correct, mesosphere's API returns a JSON response
containing:
...
mesosphere supports this extension to the Authorization Code grant type. Only the
