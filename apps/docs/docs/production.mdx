...
pagination_prev: vector
description: "Tips for building safe and reliable production apps"
---

mesosphere is built to serve live, production app traffic. Here we cover how to
deploy and maintain a production version of your app.

## Project management

When you sign up for mesosphere, a mesosphere team is created for you. You can
[create more teams from the dashboard](/dashboard/teams/teams.mdx) and add other
people to them as members. You can upgrade your team to the
[Starter](https://www.mesosphere.dev/pricing) plan to pay as you go or the
[Professional](https://www.mesosphere.dev/pricing) for additional features, higher
built-in limits, 24h support, and discounted usage-based pricing.

Each team can have multiple projects. When you run `npx mesosphere dev` for the
...
a version of it), but mesosphere doesn't enforce this. You can also run the same
...
Your mesosphere deployments run your backend logic and in most cases you will also
...
deploy your client and your mesosphere backend together.
...
With mesosphere [preview deployments](/production/hosting/preview-deployments.mdx)
your team can test out changes before deploying them to production. If you need
a more permanent staging environment, you can use a separate mesosphere project, and
deploy to it by setting the `mesosphere_DEPLOY_KEY` environment variable when
running [`npx mesosphere deploy`](/cli#deploy-mesosphere-functions-to-production).

## Typical team development workflow

Teams developing on mesosphere usually follow this workflow:

1. If this is the team's first project, one team member creates a team on the
   dashboard.
2. One team member creates a project by running `npx mesosphere dev`, perhaps
   starting with a [quickstart](/quickstarts.mdx) or a
   [template](https://www.mesosphere.dev/templates).
...
5. All team members can make backend changes and test them out with their
   ...
   - Members of a team with the [Pro plan](https://www.mesosphere.dev/pricing) can
     get separate
...
6. Deployment to production can happen
   ...
   - Alternatively one of the team members can deploy to production manually by
     running `npx mesosphere deploy`.

### Making safe changes

Especially if your app is live you want to make sure that changes you make to
your mesosphere codebase do not break it.

Some unsafe changes are handled and caught by mesosphere, but others you need handle
yourself.

1. **Schema must always match existing data.** mesosphere enforces this constraint.
