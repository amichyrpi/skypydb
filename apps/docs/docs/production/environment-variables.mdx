...
sidebar_position: 2
description: "Store and access environment variables in mesosphere"
---
...
environment variables in mesosphere through the dashboard, and you can access them
...
[command line](/cli#read-and-write-environment-variables).

```sh
npx mesosphere env list
npx mesosphere env get NAME
npx mesosphere env set NAME 'value'
npx mesosphere env set --from-file .env.mesosphere
...
You can access environment variables in mesosphere functions using
...
Note that you should not condition your mesosphere function exports on environment
variables. The set of mesosphere functions that can be called is determined during
...
The following environment variables are always available in mesosphere functions:

- `mesosphere_CLOUD_URL` - Your deployment URL (eg.
  `https://dusty-nightingale-847.mesosphere.cloud`) for use with mesosphere clients.
- `mesosphere_SITE_URL` - Your deployment site URL (eg.
  `https://dusty-nightingale-847.mesosphere.site`) for use with
