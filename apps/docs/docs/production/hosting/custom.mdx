...
or mesosphere functions from your mesosphere deployments. The settings for this feature
...
domain. The first request may take up to a minute because mesosphere will have to
mint a new SSL certificate.

Reach out to support@mesosphere.dev if you have any questions about getting set up!

<ProfessionalFeatureOnly feature="Custom domains" state="require" />

### Hosting with a Custom Domain

To use a custom domain to serve your mesosphere functions, there's an additional
step: override the `mesosphere_CLOUD_URL` environment variable.
...
Netlify, or directly running `npx mesosphere deploy --cmd 'npm run build'`. The
newly deployed code will access your mesosphere functions through your custom
domain.

The `mesosphere_CLOUD_URL` environment variable is used in several places:

- `npx mesosphere deploy --cmd '...'` sets `mesosphere_URL` (or similarly named) for
  your frontend to connect websockets and HTTP clients
- In your mesosphere functions, it is available as `process.env.mesosphere_CLOUD_URL`
- File storage URLs: `ctx.storage.getUrl(id)` and
  `ctx.storage.generateUploadUrl()`
- Generate an OpenAPI spec with `npx mesosphere function-spec --prod`

You may also override the `mesosphere_SITE_URL` environment variable to be a custom
HTTP Action domain.

- In your mesosphere functions, it is available as `process.env.mesosphere_SITE_URL`
- It may be used for webhooks
- It may be used in `auth.config.ts` as the `issuer` for mesosphere Auth

## Custom Hosting

If you're using only mesosphere for backend functionality you can host your web app
...
1. Configure <JSFileExtension name="vite.config.mts" />:
...
### Build and deploy to mesosphere and GitHub Pages

To manually deploy to GitHub pages follow these steps:

1. Checkout the branch you chose to publish from
2. Run `npx mesosphere deploy --cmd 'npm run build'` and confirm that you want to
   push your current backend code to your **production** deployment
3. Commit the build output changes and push to GitHub.

### How it works

First, `npx mesosphere deploy` runs through these steps:

1. It sets the `VITE_mesosphere_URL` (or similarly named) environment variable to
   your **production** mesosphere deployment.
...
3. It deploys your backend code, from the `mesosphere` directory, to your
