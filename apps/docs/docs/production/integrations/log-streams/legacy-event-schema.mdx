...
your [deployment's Settings](https://dashboard.mesosphere.dev/deployment/settings) >
Integrations.
...
mesosphere function logs via the `console` API.
...
- `_functionPath` =
  ...
  - Otherwise, this is a path to function within `mesosphere/` directory including
...
- `_timestamp` = Unix epoch timestamp in milliseconds
- `_functionType = "query" | "mutation" | "action" | "httpAction"`
- `_functionPath` = path to function within `mesosphere/` directory including module
- `_functionCached = true | false`. This field is only set if
  `_functionType = "query"` and says if this log event came from a cached
  function execution.
...
- `_timestamp` = Unix epoch timestamp in milliseconds.
- `message = mesosphere connection test`
