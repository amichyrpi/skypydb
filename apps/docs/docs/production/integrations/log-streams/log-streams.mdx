...
sidebar_position: 2
description: "Configure logging integrations for your mesosphere deployment"
---

Log streams enable streaming of events such as function executions and
`console.log`s from your mesosphere deployment to supported destinations, such as
Axiom, Datadog, PostHog, or a custom webhook.

The most recent logs produced by your mesosphere deployment can be viewed in the
Dashboard [Logs page](/dashboard/deployments/logs.md), the
[mesosphere CLI](/cli), or in the browser console, providing a quick and easy way
to view recent logs.
...
<ProfessionalFeatureOnly feature="Log streams" state="require" />
...
When configuring a mesosphere dataset in Axiom, a dashboard will automatically be
...
- A comma-separated list of tags that will be passed using the
  ...
  can be useful for querying or categorizing your mesosphere logs ingested by your
  Datadog deployment.
...
- `mesosphere`: An object containing metadata related to your mesosphere deployment,
...
mesosphere function logs via the [`console` API](/functions/debugging.mdx).
...
- `occ_info`: object, if the function call resulted in an OCC (write conflict
  ...
  relating to the OCC.
  [Learn more about write conflicts](https://docs.mesosphere.dev/error/#1).
...
- `usage`:
  ...
  - `file_storage_write_bytes`: number, this and `file_storage_read_bytes` make
    ...
    using the
    [storage API within mesosphere functions](/file-upload/store-files.mdx) (e.g.
...
[file URL](/file-upload/serve-files.mdx) obtained from `ctx.storage.getUrl()`.
...
bandwidth consumed within mesosphere functions themselves.
...
[History tab](https://dashboard.mesosphere.dev/deployment/history) in the dashboard.
