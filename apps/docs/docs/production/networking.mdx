...
description: "IP addresses used by mesosphere for outbound requests"
---
...
All outbound requests from your mesosphere deployment — including
[`fetch`](/functions/actions.mdx) calls in your functions (in both the mesosphere
and Node.js [runtimes](/functions/runtimes.mdx)), OIDC metadata and JWKS lookups
for [authentication](/authentication.mdx),
...
accept traffic from mesosphere, add the addresses for your deployment's
[region](/production/regions.mdx).

:::caution

These IP addresses are shared across all mesosphere deployments in a region, not
