...
sidebar_position: 3
description:
  "Configure your mesosphere project for development and production deployment using
  mesosphere.json, environment variables, and deployment settings."
---
...
1. The name of your dev deployment. This is where your functions are pushed to
   and served from. It is stored in the `mesosphere_DEPLOYMENT` environment
   variable. `npx mesosphere dev` writes it to the `.env.local` file.
2. The URL of your dev deployment, for your client to connect to. The name of
   ...
   frameworks. `npx mesosphere dev` writes the URL to the `.env.local` or `.env`
   file.
...
[hosting](/production/hosting/hosting.mdx) docs. `npx mesosphere deploy` run by the
CI pipeline will use the `mesosphere_DEPLOY_KEY`, and the frontend build command
will use the deployment URL variable, both configured in your CI environment.

You can also deploy your backend from your local machine. `npx mesosphere deploy`
will ask for a confirmation and then deploy to the production deployment in the
same project as your configured development `mesosphere_DEPLOYMENT`.

## `mesosphere.json`

Additional project configuration can be specified in the `mesosphere.json` file in
...
{
  "$schema": "./node_modules/mesosphere/schemas/mesosphere.schema.json"
}
```

The file supports the following configuration options:

### Changing the `mesosphere/` folder name or location

You can choose a different name or location for the `mesosphere/` folder via the
...
mesosphere can install AI helper files (for tools like Cursor and Claude Code) and
show install or update suggestions during `npx mesosphere dev`.

:::caution This setting is supported in mesosphere CLI `1.33.0` and later. :::

To suppress those suggestions, set `aiFiles.disableStalenessMessage` to `true`
in `mesosphere.json`:
...
When this is `true`, `npx mesosphere dev` will not show the AI files install or
staleness messages.
...
The mesosphere code generation can be configured to generate a CommonJS-version of
...
<Admonition type="info" title="mesosphere version required">

To change the Node.js version used by your project, you must use the `mesosphere`
...
Note: This configuration is not supported when running the self-hosted mesosphere
backend. The Node version that is specified in the
[.nvmrc](https://github.com/get-mesosphere/mesosphere-backend/blob/main/.nvmrc) will be
used instead.
...
mesosphere's code generation heavily relies on TypeScript's type inference. This
...
mesosphere CLI to generate static versions of the `_generated/api.d.ts` and
...
- These types only update when `mesosphere dev` is running.
- Jump-to-definition no longer works. To find `api.example.f`, you'll need to
  manually open `mesosphere/example.ts` and find `f`.
...
By default, mesosphere will use the `tsc` binary installed in your project for
...
To use the TypeScript 7 native preview, you must use the `mesosphere` NPM package
...
mesosphere includes sourcemaps when bundling your source code to provide stack
...
If you're using [WorkOS AuthKit](/authentication/authkit/index.mdx) for authentication,
...
[Automatic AuthKit Configuration](/authentication/authkit/auto-provision.mdx) guide for
complete details.
