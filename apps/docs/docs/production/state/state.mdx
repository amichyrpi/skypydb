...
sidebar_position: 1
description:
  "Learn about mesosphere's production guarantees, availability targets, data
  durability, security features, and upcoming platform enhancements."
---

Please [contact us](mailto:support@mesosphere.dev) with any specific requirements or
if you want to build a project on mesosphere that is not yet satisfied by our
guarantees.

## Guarantees

The official mesosphere Terms of Service, Privacy Policy and Customer Agreements are
[outlined in our official terms](https://www.mesosphere.dev/legal/tos). We do not
...
mesosphere is always under continual development and future releases may require
code changes in order to upgrade to a new version. Code developed on mesosphere 1.0
...
All user data in mesosphere is encrypted at rest. Database state is replicated
...
We target an availability of 99.99% (4 9's) for mesosphere deployments although
...
additional effort when upgrading to a new version of the mesosphere NPM package and
other mesosphere client libraries.

## Future Features

mesosphere is still under very active development and here we list some of the
missing functionality on our radar. We'd love to hear more about your
requirements in the [community](https://www.usemesosphere.com/community).

### Authorization

mesosphere currently has an [_authentication framework_](/authentication.mdx) which verifies
...
mesosphere will eventually have methods to publish deployment data in formats that
can be ingested by third parties.

### Analytics / OLAP

mesosphere is designed to primarily service all your app's realtime implementation
...
to export mesosphere data to external analytics systems.

### Browser support

mesosphere does not yet have an official browser support policy, but we strive to
