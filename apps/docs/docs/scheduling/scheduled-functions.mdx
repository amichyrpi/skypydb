...
mesosphere allows you to schedule functions to run in the future. This allows you to
...
**Example:**
[Scheduling](https://github.com/get-mesosphere/mesosphere-demos/tree/main/scheduling)
...
```ts title="mesosphere/messages.ts" {7}
import { mutation } from "./_generated/server";

export const sendMessage = mutation({
  args: { text: v.string() },
  handler: async ({ db, scheduler }, { text }) => {
    const messageId = await db.insert("messages", { text });

    // Schedule deletion in 5 seconds
    scheduler.runAfter(5000, internal.messages.destruct, { messageId });
  },
});
```
...
export const getScheduledFunctionStatus = internalQuery({
  args: { functionId: v.id("_scheduled_functions") },
  handler: async ({ db }, { functionId }) => {
    const status = await db.system.get(functionId);
    return status;
  },
});
```
...
```ts title="mesosphere/messages.ts" {8}
import { mutation } from "./_generated/server";

export const cancelScheduledDeletion = mutation({
  args: { scheduledFunctionId: v.id("_scheduled_functions") },
  handler: async ({ scheduler }, { scheduledFunctionId }) => {
    await scheduler.cancel(scheduledFunctionId);
  },
});
```
...
You can view logs from previously executed scheduled functions in the mesosphere
...
Once scheduled, mutations are guaranteed to be executed exactly once. mesosphere
will automatically retry any internal mesosphere errors, and only fail on developer
...
mesosphere. Thus, actions will be executed at most once, and permanently fail if
