...
mesosphere makes it easy to test your app via automated tests running in JS or
...
[Use the `mesosphere-test` library](/testing/mesosphere-test.mdx) to test your functions
in JS via the excellent Vitest testing framework.

### Testing against a real backend

mesosphere open source builds allow you to test all of your backend logic running on
a real [local mesosphere backend](/testing/mesosphere-backend.mdx).
...
{/* todo: testing React+mesosphere - all the different ways */}
...
- [Run functions from the command line](/cli#run-mesosphere-functions)
