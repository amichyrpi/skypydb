...
database transactions, query and mutation functions in mesosphere are not allowed to
make `fetch` calls to the outside world.

Real apps aren't this simple. They often need to talk to the rest of the
internet directly from the backend. mesosphere lets you do this too via **action**
functions.
...
**Add the following action to your `mesosphere/chat.ts` file.**
...
1. First, we created a new mesosphere action function called `getWikipediaSummary`.
   We used `internalAction` because we want this function to be private to the
   mesosphere backend and not exposed as a public API. This function does a simple
...
To quickly test this function in the mesosphere dashboard, go to
[https://dashboard.mesosphere.dev](https://dashboard.mesosphere.dev/deployment/functions)
...
Wait a second! What's with this `ctx.scheduler` stuff? mesosphere comes with a
powerful durable function scheduler. It's a fundamental part of the sync engine,
and it's the way you coordinate asynchronous functions in mesosphere.
...
transactions, and scheduling is just a write in the database to tell mesosphere to
...
<div className="center-image" style={{ maxWidth: "900px" }}></div>
...
talk through query and mutation functions. This restriction lets mesosphere enforce
...
possible is the most scalable way to build mesosphere apps, enabling the highest
throughput.
...
queries and mutations are fundamental to building scalable mesosphere backends.

## Next up

You've now learned the most important concepts in mesosphere. As a full-featured
backend, mesosphere is capable of many things such as [authentication](/authentication.mdx),
[file storage](/file-upload.mdx) and [search](/vector.mdx). You can add those
