...
Full text search allows you to find mesosphere documents that approximately match a
search query.
...
**Example:**
[Search App](https://github.com/get-mesosphere/mesosphere-demos/tree/main/search)
...
Search indexes are built and queried using mesosphere's multi-segment search
...
querying. Search indexes are defined as part of your mesosphere
...
method defines which search index to query and how mesosphere will use that search
...
description of which documents mesosphere should consider when running the query.
...
mesosphere will break up the query into separate words (called _terms_) and
...
less likely to hit mesosphere's limits because mesosphere will use the search index to
...
mesosphere full-text search is designed to power as-you-type search experiences. In
...
the exact rules mesosphere applies is subject to change to improve the quality of
search results.

Search queries return results in relevance order. Internally, mesosphere ranks the
...
[source code](https://github.com/get-mesosphere/mesosphere-backend/blob/main/crates/vector/src/constants.rs).
