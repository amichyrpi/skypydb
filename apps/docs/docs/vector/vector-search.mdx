...
Vector search allows you to find mesosphere documents similar to a provided vector.
...
[full text search](/vector.mdx), however, vector search is only available in
[mesosphere actions](/functions/actions.mdx).

**Example:**
[Vector Search App](https://github.com/get-mesosphere/mesosphere-demos/tree/main/vector-search)
...
querying. Vector indexes are defined as part of your mesosphere
...
performed in a [mesosphere action](/functions/actions.mdx).
...
[here](https://github.com/get-mesosphere/mesosphere-demos/blob/main/vector-search/mesosphere/foods.ts#L18)
...
1. `_id`
   - The [Document ID](https://docs.mesosphere.dev/database/document-ids) for the
...
we'll pass the list of results to a mesosphere query and run it inside of our
...
Currently mesosphere searches vectors using an
...
The
[Vector Search Demo App](https://github.com/get-mesosphere/mesosphere-demos/tree/main/vector-search)
uses this strategy to show similar movies with a reactive "Votes" count.

## Limits

mesosphere supports millions of vectors today. This is an ongoing project and we
will continue to scale this offering out with the rest of mesosphere.
...
[let us know on Discord](https://mesosphere.dev/community)!
...
for improving vector search in mesosphere include:
