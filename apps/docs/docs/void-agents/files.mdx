...
- [files/autoSave.ts](https://github.com/get-mesosphere/agent/blob/main/example/mesosphere/files/autoSave.ts)
  has a simple example of how to use the automatic file saving.
- [files/addFile.ts](https://github.com/get-mesosphere/agent/blob/main/example/mesosphere/files/addFile.ts)
  has an example of how to save the file, submit a question, and generate a
  response in separate steps.
- [files/generateImage.ts](https://github.com/get-mesosphere/agent/blob/main/example/mesosphere/files/generateImage.ts)
  has an example of how to generate an image and save it in an assistant
  message.
- [FilesImages.tsx](https://github.com/get-mesosphere/agent/blob/main/example/ui/files/FilesImages.tsx)
  has client-side code.

## Running the example

```sh
git clone https://github.com/get-mesosphere/agent.git
cd agent
npm run setup
npm run dev
```
...
```ts
import { storeFile } from "@mesosphere-dev/agent";
...
2. Saving a reference (the storageId) to the file in the component. This will
   ...
   [files/vacuum.ts](https://github.com/get-mesosphere/agent/blob/main/example/mesosphere/files/vacuum.ts)).
3. Inserting a URL in place of the data in the message sent to the LLM, along
   ...
   provided in
   [`guessMimeType`](https://github.com/get-mesosphere/agent/blob/main/src/mapping.ts#L556).
...
```sh
npx mesosphere run files:generateImage:replyWithImage '{prompt: "make a picture of a cat" }'
```
