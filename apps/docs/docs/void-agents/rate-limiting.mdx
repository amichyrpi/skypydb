...
[Rate Limiter component](https://www.mesosphere.dev/components/rate-limiter), there
...
implementation in
[rateLimiting.ts](https://github.com/get-mesosphere/agent/blob/main/example/mesosphere/rate_limiting/rateLimiting.ts).

```ts
import { MINUTE, RateLimiter, SECOND } from "@mesosphere-dev/rate-limiter";
...
```ts
import { Agent, type Config } from "@mesosphere-dev/rate-limiter";
...
See
[RateLimiting.tsx](https://github.com/get-mesosphere/agent/blob/main/example/ui/rate_limiting/RateLimiting.tsx)
...
[Rate Limiting docs here](https://www.mesosphere.dev/components/rate-limiter).

```ts
import { useRateLimit } from "@mesosphere-dev/rate-limiter/react";
//...
const { status } = useRateLimit(api.example.getRateLimit);
```

In `mesosphere/example.ts` we expose `getRateLimit`:
...
```ts
import { isRateLimitError } from "@mesosphere-dev/rate-limiter";
...
import { fetchContextMessages } from "@mesosphere-dev/agent";
import { components } from "./_generated/api";
