...
example in
[this demo](https://github.com/get-mesosphere/agent/blob/main/example/mesosphere/usage_tracking/usageHandler.ts)
...
With an associated schema in `mesosphere/schema.ts`:
...
See
[usage_tracking/invoicing.ts](https://github.com/get-mesosphere/agent/blob/main/example/mesosphere/usage_tracking/invoicing.ts)
for an example of how to generate invoices.

You can then add it to `mesosphere/crons.ts`:
