...
function resolve_api_url(
  ...
  if (mode === "cloud") {
    const cloud_url = dependencies.env_get("MESOSPHERE_CLOUD_API_URL");
    if (typeof cloud_url === "string" && cloud_url.trim().length > 0) {
      return normalize_api_url(cloud_url);
    }
    const fallback_url = dependencies.env_get("MESOSPHERE_API_URL");
  ...
  const local_url = dependencies.env_get("MESOSPHERE_API_URL");
...
function resolve_api_key(
  ...
  const env_api_key = dependencies.env_get("MESOSPHERE_API_KEY");
...
export async function run_deploy_command(
  ...
  const source_input = options.source?.trim() || "mesosphere";
  ...
  if (!api_url) {
    dependencies.error(
      "Cloud deploy requires --api-url (or MESOSPHERE_CLOUD_API_URL / MESOSPHERE_API_URL).",
    );
    return 1;
  }
  const api_key = resolve_api_key(mode, options, dependencies);
  if (!api_key) {
    dependencies.error("Deploy requires --api-key (or MESOSPHERE_API_KEY).");
    return 1;
  }
