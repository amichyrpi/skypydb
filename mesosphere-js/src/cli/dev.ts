...
const LOCAL_SOURCE_DIR = "mesosphere";
...
async function deploy_local_if_functions_exist(
  ...
  if (snapshot.deployable_function_count === 0) {
    dependencies.log(
      "No readFunction/writeFunction exports found yet in ./mesosphere. Waiting for new functions...",
    );
    return 0;
  }
...
async function watch_and_auto_deploy_local(
  ...
  dependencies.log(
    "Watching ./mesosphere for new or changed functions. Press Ctrl+C to stop.",
  );
