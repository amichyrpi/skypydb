...
const writeState = isClient
  ? callwrite(api.message.newMessage)
  : {
      newdata: async () => ({ error: new Error("Client-only write") }),
      error: ref(null),
    };

const { data: messages, notready, error: readError } = readState;
const { newdata: sendMessage, error: writeError } = writeState;
...
const clerkInstance = ref<any>(null);
...
onMounted(async () => {
  const publishableKey = runtimeConfig.public.clerkPublishableKey as string;
  if (!publishableKey) {
    clerkError.value = "Missing NUXT_PUBLIC_CLERK_PUBLISHABLE_KEY.";
    return;
  }
