  ...
  async delete(params: DeleteParams = {}): Promise<void> {
    ...
    if (by_ids !== undefined) {
      if (ids !== undefined && JSON.stringify(ids) !== JSON.stringify(by_ids)) {
        throw new Error("Use either 'ids' or legacy 'by_ids', not conflicting values for both.");
      }
      ids = by_ids;
    }
    ...
    if (by_metadatas !== undefined) {
      ...
      if (!Array.isArray(by_metadatas) && typeof by_metadatas === "object") {
        ...
        } else if (by_metadatas.length > 1 && by_metadatas.every((item) => typeof item === "object")) {
    ...
    if (by_data !== undefined) {
      if (where_document !== undefined) {
        throw new Error("Use either 'where_document' or legacy 'by_data', not both.");
      }
      if (typeof by_data === "string") {
        where_document = { $contains: by_data };
      } else if (Array.isArray(by_data)) {
        const values = by_data.filter((value): value is string => typeof value === "string" && value.length > 0);
        if (values.length === 1) {
    ...
    if (ids === undefined && where === undefined && where_document === undefined) {
      throw new Error(
