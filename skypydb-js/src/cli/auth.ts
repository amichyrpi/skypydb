...
export function configure_local_auth_env(dependencies: CliDependencies): number {
  ...
  try {
    ...
    const api_url =
      dependencies.env_get("SKYPYDB_API_URL")?.trim() || LOCAL_API_URL_DEFAULT;
    const api_key =
      dependencies.env_get("SKYPYDB_API_KEY")?.trim() || LOCAL_API_KEY_DEFAULT;

    const with_api_url = upsert_env_key(current_content, "SKYPYDB_API_URL", api_url);
    const with_api_key = upsert_env_key(with_api_url, "SKYPYDB_API_KEY", api_key);
    dependencies.write_atomic(env_path, with_api_key);
    dependencies.log("Configured local SKYPYDB API URL and API key in .env");
    return 0;
  } catch (error) {
