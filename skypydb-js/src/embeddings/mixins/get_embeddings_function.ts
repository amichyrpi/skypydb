...
function validate_remaining_config(provider: string, config: Record<string, unknown>): void {
...
export function get_embedding_function(
  ...
  if (normalized === "ollama") {
    ...
    const base_url = (mutable.base_url as string | undefined) ?? "http://localhost:11434";
    ...
    const provider_instance = new OllamaEmbedding({ model, base_url, dimension });
    return provider_instance.embed.bind(provider_instance);
  }

  if (normalized === "openai") {
    const api_key = mutable.api_key as string | undefined;
    const model = (mutable.model as string | undefined) ?? "text-embedding-3-small";
  ...
  if (normalized === "sentence-transformers" || normalized === "sentence-transformer") {
