...
export class HttpCollection {
  ...
  async add(params: AddParams): Promise<void> {
    ...
    if (!embeddings) {
      if (!documents) {
        throw new Error("Either embeddings or documents/data must be provided.");
      }
    ...
    if (params.metadatas !== undefined && params.metadatas.length !== ids.length) {
      throw new Error(
  ...
  async query(params: QueryParams = {}): Promise<QueryResult> {
    ...
    for (let query_index = 0; query_index < response.ids.length; query_index += 1) {
      ...
      for (let index = 0; index < row_ids.length; index += 1) {
        ...
        if (
          matches_vector_filters(
            candidate,
            params.where,
            params.where_document,
          )
        ) {
  ...
  async delete(params: DeleteParams = {}): Promise<void> {
    ...
    const ids_to_delete = rows
      .filter((row) => {
        if (
          normalized.by_data_values &&
          normalized.by_data_values.length > 1
        ) {
...
export class RelationalTableClient {
  ...
  async count(params: Pick<RelationalQueryParams, "where"> = {}): Promise<number> {
...
export class SchemaClient {
  ...
  async get(): Promise<SchemaReadResponse> {
    return this.transport.request<SchemaReadResponse>("GET", "/v1/admin/schema");
  }
}
