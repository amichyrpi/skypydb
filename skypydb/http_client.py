"""HTTP-only Skypydb client for vector, relational, and schema APIs."""
...
def _matches_operator(metadata_value: Any, operator: str, operator_value: Any) -> bool:
    ...
    if operator == "$ne":
        return metadata_value != operator_value
    if operator == "$gt":
        return isinstance(metadata_value, (int, float)) and isinstance(
            operator_value, (int, float)
...
def _matches_where(metadata: Optional[Dict[str, Any]], where: Optional[Dict[str, Any]]) -> bool:
    if where is None:
        return True
...
def _apply_paging(rows: List[Any], limit: Optional[int], offset: Optional[int]) -> List[Any]:
        ...
        *,
        ...
        if embeddings is None:
            if documents is None:
                raise ValueError("Either embeddings or documents/data must be provided.")
    ...
    def get(
        ...
        *,
        ...
        return {
            ...
            "embeddings": None,
            "documents": [row.get("document") for row in paged]
            if "documents" in include
            else None,
            "metadatas": [row.get("metadata") for row in paged]
    ...
    def query(
        ...
        *,
        ...
        for query_index, id_row in enumerate(response.get("ids", [])):
            doc_row = response.get("documents", [])[query_index] if response.get("documents") else []
            meta_row = response.get("metadatas", [])[query_index] if response.get("metadatas") else []
            dist_row = response.get("distances", [])[query_index] if response.get("distances") else []
            ...
            for index, item_id in enumerate(id_row):
                ...
                if _matches_where(candidate["metadata"], where) and _matches_where_document(
                    candidate["document"], where_document
                ):
        ...
        *,
        ...
        if by_ids is not None:
            if ids is not None and ids != by_ids:
                raise ValueError("Use either 'ids' or legacy 'by_ids', not both.")
            ids = by_ids
        ...
        ids_to_delete = []
        for row in rows:
            if by_data_values and len(by_data_values) > 1:
                if any(value in (row.get("document") or "") for value in by_data_values):
                    ids_to_delete.append(row["id"])
            elif _matches_where(row.get("metadata"), where) and _matches_where_document(
                row.get("document"), where_document
            ):
                ids_to_delete.append(row["id"])

        if not ids_to_delete:
            if where is None and where_document is None and by_data_values is None:
                raise ValueError(
...
class RelationalTableClient:
    """Relational table operations over HTTP."""

    def __init__(self, transport: _HttpTransport, table_name: str):
        self._transport = transport
        self._table_name = table_name

    def insert(self, value: Dict[str, Any]) -> str:
        data = self._transport.request(
            "POST",
            f"/v1/relational/{_encode_segment(self._table_name)}/insert",
            {"value": value},
        )
        return str(data["id"])
    ...
    def update(
        ...
        *,
        ...
        data = self._transport.request(
            "POST",
            f"/v1/relational/{_encode_segment(self._table_name)}/update",
            {"id": id, "where": where, "value": value},
        )
        return int(data["affected_rows"])
    ...
    def delete(
        ...
        *,
        ...
        data = self._transport.request(
            "POST",
            f"/v1/relational/{_encode_segment(self._table_name)}/delete",
            {"id": id, "where": where},
        )
        return int(data["affected_rows"])
    ...
    def move(
        ...
        *,
        ...
        data = self._transport.request(
            "POST",
            f"/v1/relational/{_encode_segment(self._table_name)}/move",
            {
                "to_table": to_table,
                "id": id,
                "where": where,
    ...
    def query(
        ...
        *,
        ...
        data = self._transport.request(
            "POST",
            f"/v1/relational/{_encode_segment(self._table_name)}/query",
            {
                "where": where,
                "order_by": order_by if order_by is not None else orderBy,
                "limit": limit,
                "offset": offset,
            },
        )
    ...
    def count(self, *, where: Optional[Dict[str, Any]] = None) -> int:
        data = self._transport.request(
            "POST",
            f"/v1/relational/{_encode_segment(self._table_name)}/count",
            {"where": where},
        )
    ...
    def first(
        ...
        *,
        ...
        data = self._transport.request(
            "POST",
            f"/v1/relational/{_encode_segment(self._table_name)}/first",
            {
                "where": where,
                "order_by": order_by if order_by is not None else orderBy,
                "limit": limit,
...
class SchemaClient:
    """Schema admin API wrapper."""

    def __init__(self, transport: _HttpTransport):
        self._transport = transport

    def apply(self, schema: Dict[str, Any]) -> Any:
        return self._transport.request("POST", "/v1/admin/schema/apply", {"schema": schema})

    def validate(self, schema: Dict[str, Any]) -> Dict[str, Any]:
        return self._transport.request("POST", "/v1/admin/schema/validate", {"schema": schema})
...
def httpClient(**kwargs: Any) -> HttpClient:
    """Factory alias for `HttpClient`."""
...
class AsyncRelationalTableClient:
    """Async wrapper around `RelationalTableClient`."""

    def __init__(self, table: RelationalTableClient):
        self._table = table

    async def insert(self, value: Dict[str, Any]) -> str:
        return await asyncio.to_thread(self._table.insert, value)

    async def update(self, **kwargs: Any) -> int:
        return await asyncio.to_thread(self._table.update, **kwargs)

    async def delete(self, **kwargs: Any) -> int:
        return await asyncio.to_thread(self._table.delete, **kwargs)

    async def move(self, **kwargs: Any) -> int:
        return await asyncio.to_thread(self._table.move, **kwargs)

    async def query(self, **kwargs: Any) -> List[Dict[str, Any]]:
        return await asyncio.to_thread(self._table.query, **kwargs)

    async def count(self, **kwargs: Any) -> int:
        return await asyncio.to_thread(self._table.count, **kwargs)

    async def first(self, **kwargs: Any) -> Optional[Dict[str, Any]]:
        return await asyncio.to_thread(self._table.first, **kwargs)


class AsyncSchemaClient:
    """Async wrapper around `SchemaClient`."""

    def __init__(self, schema_client: SchemaClient):
        self._schema = schema_client

    async def apply(self, schema: Dict[str, Any]) -> Any:
        return await asyncio.to_thread(self._schema.apply, schema)

    async def validate(self, schema: Dict[str, Any]) -> Dict[str, Any]:
        return await asyncio.to_thread(self._schema.validate, schema)
...
class AsyncHttpClient:
    ...
    async def create_collection(
        ...
        collection = await asyncio.to_thread(self._client.create_collection, name, metadata)
    ...
    async def get_or_create_collection(
        ...
        collection = await asyncio.to_thread(self._client.get_or_create_collection, name, metadata)
        return AsyncHttpCollection(collection)
...
def AsynchttpClient(**kwargs: Any) -> AsyncHttpClient:
    """Backward-compatible async factory alias."""
